...
# Чтение данных из файла wallet.csv
wallets_and_proxies = []
with open('data/wallet.csv', 'r', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    for row in reader:
        wallets_and_proxies.append((row['wallet_address'].strip(), row['proxy'].strip()))

# Чтение резервных прокси из reserv_proxy.csv
reserv_proxies = []
with open('data/reserv_proxy.csv', 'r', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    for row in reader:
        reserv_proxies.append(row['proxy'].strip())

for wallet, proxy_url in wallets_and_proxies:
    print(f"\n[INFO] Проверка для кошелька: {wallet} через прокси: {proxy_url}")

    # Новый запрос к API
    blockscout_url = f"https://soneium.blockscout.com/api/v2/addresses/{wallet}/nft/collections?type=ERC-1155"
    blockscout_headers = {
        "accept": "*/*",
    ...
    while True:
        ...
        try:
            ...
            if reserv_proxies:
                proxy_url = random.choice(reserv_proxies)
                print(f"[INFO] Замена прокси на: {proxy_url}")
            else:
                print("[ERROR] Нет доступных резервных прокси.")
                break
        except ValueError:
